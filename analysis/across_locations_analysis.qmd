...
```
### CCN recognizability plot
```{r rec_single, fig.width=6, fig.height=6}
rec_single <- ggplot(data = df_part,
                     aes(color = location, fill = location, x = age, y = recognizability)) +
  # stat_smooth(method = "loess", geom = "ribbon", alpha = 0.1, span = 1, color = NA) +
  stat_smooth(geom = "line", alpha = 0.3, linewidth = 0.8, span = 1, se = FALSE) +
  scale_alpha_identity() +
  geom_pointrange(data = df_age_rec,
                  aes(x = age, 
                      y = mean_value,
                      ymin = ci_lower, 
                      ymax = ci_upper),
  ...
  labs(x = "Age (years)", y = "Recognizability") +
...
ggsave(here("data/figures/recognizability_single.pdf"),
       rec_single, bg = "white", width = 8, height = 6)
```
